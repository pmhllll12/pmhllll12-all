...
- `app/use_cases/langgraph_interactor.py`가 `app/use_cases/langchain_morningstar_interactor.py`(003 소관, 단순 LCEL 체인)와 나란히 스텁으로 존재한다는 것은 "조건 분기·재시도가 필요한 흐름이 생기면 LangGraph로 옮긴다"는 신호로 읽는다 — §2에서 그 조건을 먼저 확인한다.
...
- **Checkpointer**: 대화·작업 상태를 세션 간 이어가려면 필요하지만, 지금 이 앱에는 그 요구가 확인되지 않았다 — 도입하지 않는다(YAGNI). 필요해지는 시점에 이 섹션에 구체적 조건과 함께 추가한다.
...
`langgraph_interactor.py`를 실제로 구현하기 전에 이 표로 필요성부터 확인한다 — 위 왼쪽 열(단순 순차 흐름)에 해당하면 `langgraph_interactor.py`를 채우지 말고 003의 인터랙터로 대체하거나, 빈 스텁을 제거한다. 빈 스텁을 "일단 만들어 뒀으니 채운다"는 이유만으로 구현하지 않는다.
...
- [ ] `langgraph_interactor.py` 구현 전에 §2 표로 LangGraph가 실제로 필요한지 확인했다(필요 없으면 스텁을 구현하지 않거나 제거한다).
...
- [ ] Checkpointer 등 상태 영속 기능은 실제 요구가 확인되기 전에는 추가하지 않았다.
